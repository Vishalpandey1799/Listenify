...
export const docxnpdfparser = async(file,type) =>{
      if(!file || !file.buffer){
         return;
      }
     ...
     try {
        ...
        if(type==="pdf"){
         let parsedPdf = await PdfParse(file.buffer);
         // finalOutput= striptags(parsedPdf.text);
         finalOutput = parsedPdf.text;
          return finalOutput;
        } else if(type==="docx"){
          let parsedDocx = await mammoth.extractRawText(file.buffer);
          finalOutput = parsedDocx.value;
          return finalOutput;
        }else{
         return null
        }
